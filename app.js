...
function handleRealtimeGameChange(change) {
  ...
  // If this is our current game, update the live scores
  if (state.currentGameId && gameData.id === state.currentGameId) {
    // Another user updated the game - sync if we're a viewer
    if (viewerMode) {
      syncGameFromDB(gameData);
    }
  }

  // Refresh games list if open
...
const state = {
  quarter: '1',
  home: { name:'HOME', dbId:null, score:0, fouls:0, timeouts:5, players:[], pto:0, fbp:0, twocp:0 },
  away: { name:'AWAY', dbId:null, score:0, fouls:0, timeouts:5, players:[], pto:0, fbp:0, twocp:0 },
  selectedPlayer: null,
  history: [],
  plays: [],
  statsView: 'home',
...
let viewerMode      = false; // true = just watching a game, not recording
...
async function createGame() {
  ...
  const rows = await db.insert('games', {
    ...
    home_pto:0, home_fbp:0, home_twocp:0,
    away_pto:0, away_fbp:0, away_twocp:0,
...
function makeStatRow(p, team) {
  return { game_id:state.currentGameId, player_id:p.dbId, team_id:state[team].dbId,
    ...
    pto:p.pto||0, fbp:p.fbp||0, twocp:p.twocp||0 };
}
...
async function saveGameToDB() {
  ...
  try {
    await db.update('games', {
      ...
      home_pto:state.home.pto, home_fbp:state.home.fbp, home_twocp:state.home.twocp,
      away_pto:state.away.pto, away_fbp:state.away.fbp, away_twocp:state.away.twocp,
  ...
  try {
    if (tab === 'boxscore') {
      ...
      let html = `<div style="overflow-x:auto"><table class="stats-table" style="width:100%"><thead><tr>
        ...
        <th style="color:#ce93d8">PTO</th><th style="color:#ce93d8">FBP</th><th style="color:#ce93d8">2CP</th>
      </tr></thead><tbody>`;
      stats.forEach(s => {
        ...
        html += `<tr>
          ...
          <td style="color:#ce93d8">${s.pto||0}</td><td style="color:#ce93d8">${s.fbp||0}</td><td style="color:#ce93d8">${s.twocp||0}</td>
        </tr>`;
      });
...
async function loadGameIntoScorer(gameId) {
  ...
  try {
    ...
    if (g.home_team_id) {
      ...
      state.home.name  = g.home_name || 'HOME';
      state.home.dbId  = g.home_team_id;
      state.home.score = g.home_score || 0;
      state.home.fouls = g.home_fouls || 0;
      state.home.timeouts = g.home_timeouts ?? 5;
      state.home.players = homePlayers.map(p => ({ id:playerIdCounter++, dbId:p.id, dbStatId:null, num:p.num, name:p.name, pos:p.pos, onCourt:false, pts:0, fgm:0, fga:0, tpm:0, tpa:0, ftm:0, fta:0, or:0, dr:0, ast:0, stl:0, blk:0, to:0, fls:0, pto:0, fbp:0, twocp:0 }));
    }
    if (g.away_team_id) {
      const awayPlayers = await loadPlayersFromDB(g.away_team_id);
      state.away.name  = g.away_name || 'AWAY';
      state.away.dbId  = g.away_team_id;
      state.away.score = g.away_score || 0;
      state.away.fouls = g.away_fouls || 0;
      state.away.timeouts = g.away_timeouts ?? 5;
      state.away.players = awayPlayers.map(p => ({ id:playerIdCounter++, dbId:p.id, dbStatId:null, num:p.num, name:p.name, pos:p.pos, onCourt:false, pts:0, fgm:0, fga:0, tpm:0, tpa:0, ftm:0, fta:0, or:0, dr:0, ast:0, stl:0, blk:0, to:0, fls:0, pto:0, fbp:0, twocp:0 }));
    }
    ...
    if (viewerMode) {
      toast('👁 Viewing live game — scores sync automatically');
    } else {
      toast(`✓ Loaded game: ${g.home_name} vs ${g.away_name}`);
    }
  } catch(e) { toast('❌ ' + e.message); }
  finally { hideLoading(); }
}
...
async function loadTeamsView() {
  ...
  try {
    ...
    for (const t of teams) {
      ...
      grid.appendChild(buildTeamCard(t, players));
    }
...
document.querySelectorAll('[data-sort]').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    btn.classList.add('active');
    currentGamesSort = btn.dataset.sort;
...
async function loadStandings(sortBy = 'wins') {
  ...
  try {
    // Build standings from finished games
    const teams   = await loadTeamsFromDB();
    const games   = await db.select('games', { 'status':'eq.finished', 'select':'*' });

    const standMap = {};
    teams.forEach(t => {
      standMap[t.id] = { id:t.id, name:t.name, gp:0, w:0, l:0, pf:0, pa:0 };
    });

    games.forEach(g => {
      const home = standMap[g.home_team_id];
      const away = standMap[g.away_team_id];
    ...
    tbody.innerHTML = '';
    rows.forEach((r, i) => {
      const pct  = r.gp ? (r.w/r.gp*100).toFixed(1) : '0.0';
...
async function renderSavedTeams() {
  ...
  try {
    ...
    for (const t of teams) {
      ...
      `;
      card.querySelector('.ts-load-btn').addEventListener('click', async e => {
        e.stopPropagation();
        await loadTeamFromDB(e.currentTarget.dataset.id, e.currentTarget.dataset.name);
      });
...
async function loadTeamFromDB(teamDbId, name) {
  ...
  try {
    ...
    state[setupTeam].players = players.map(p => ({
      id:playerIdCounter++, dbId:p.id, dbStatId:null,
      num:p.num, name:p.name, pos:p.pos, onCourt:false,
      pts:0, fgm:0, fga:0, tpm:0, tpa:0, ftm:0, fta:0,
      or:0, dr:0, ast:0, stl:0, blk:0, to:0, fls:0, pto:0, fbp:0, twocp:0,
    }));
    updateTeamNameDisplay(setupTeam);
    renderRoster(setupTeam); renderStats();
    closeTeamSetup();
    toast(`✓ Loaded ${name} — ${players.length} players`);
  } catch(e) { toast('❌ '+e.message); }
  finally { hideLoading(); }
}
...
async function addPlayerFromModal() {
  ...
  const num = $('managePlayerNum').value.trim() || '—';
  const pos = $('managePlayerPos').value;
  ...
  try {
    ...
    state[manageTeam].players.push({ id:playerIdCounter++, dbId, dbStatId:null, num, name, pos, onCourt:onCourtCount<5, pts:0, fgm:0, fga:0, tpm:0, tpa:0, ftm:0, fta:0, or:0, dr:0, ast:0, stl:0, blk:0, to:0, fls:0, pto:0, fbp:0, twocp:0 });
...
function renderManageList() {
  const list = $('managePlayerList');
  const players = state[manageTeam].players;
  ...
  players.forEach(p => {
    ...
    row.className = 'manage-player-row';
    row.innerHTML = `
...
function confirmSub() {
  ...
  closeSubModal(); renderRoster('home'); renderRoster('away');
  scheduleSaveGame();
}
...
document.querySelectorAll('.sq-btn').forEach(btn => {
  btn.addEventListener('click', ()=>{
    const team=btn.dataset.team, pts=parseInt(btn.dataset.pts);
    const snap=captureSnapshot();
    state[team].score+=pts; updateScore(team); bumpScore(team);
    addPlay(team,`${teamName(team)} +${pts}pt`,team,pts);
...
document.querySelectorAll('.act-btn').forEach(btn => {
  btn.addEventListener('click', ()=>{
    ...
    const snap = captureSnapshot();
    switch(action) {
      case '2pm':       p.fgm++;p.fga++;p.pts+=2;state[team].score+=2;bumpScore(team);addPlay(team,`${p.name} — 2PT Made`,team,2,p.dbId);break;
      case '2pm_miss':  p.fga++;addPlay(team,`${p.name} — 2PT Miss`,team,0,p.dbId);break;
      case '3pm':       p.tpm++;p.tpa++;p.fgm++;p.fga++;p.pts+=3;state[team].score+=3;bumpScore(team);addPlay(team,`${p.name} — 3PT Made`,team,3,p.dbId);break;
      case '3pm_miss':  p.tpa++;p.fga++;addPlay(team,`${p.name} — 3PT Miss`,team,0,p.dbId);break;
      case 'ftm':       p.ftm++;p.fta++;p.pts+=1;state[team].score+=1;bumpScore(team);addPlay(team,`${p.name} — Free Throw`,team,1,p.dbId);break;
      ...
      case 'pto':       if(!p.pto)p.pto=0;p.pto++;p.pts+=2;p.fgm++;p.fga++;state[team].score+=2;state[team].pto=(state[team].pto||0)+2;bumpScore(team);updateSpecialStats();addPlay(team,`${p.name} — PTO +2`,'special',2,p.dbId);break;
      case 'fbp':       if(!p.fbp)p.fbp=0;p.fbp++;p.pts+=2;p.fgm++;p.fga++;state[team].score+=2;state[team].fbp=(state[team].fbp||0)+2;bumpScore(team);updateSpecialStats();addPlay(team,`${p.name} — FBP +2`,'special',2,p.dbId);break;
      case '2cp':       if(!p.twocp)p.twocp=0;p.twocp++;p.pts+=2;p.fgm++;p.fga++;state[team].score+=2;state[team].twocp=(state[team].twocp||0)+2;bumpScore(team);updateSpecialStats();addPlay(team,`${p.name} — 2CP +2`,'special',2,p.dbId);break;
      default: return;
    }
...
function updateSpecialStats() {
  dom.homePTO.textContent=state.home.pto; dom.awayPTO.textContent=state.away.pto;
  dom.homeFBP.textContent=state.home.fbp; dom.awayFBP.textContent=state.away.fbp;
  dom.home2CP.textContent=state.home.twocp; dom.away2CP.textContent=state.away.twocp;
}
...
function renderStats() {
  ...
  if (!players.length){dom.statsBody.innerHTML=`<tr><td colspan="21" style="text-align:center;color:var(--muted);padding:18px;font-size:.8rem">No players</td></tr>`;return;}
  dom.statsBody.innerHTML=players.map(p=>`<tr>
    ...
    <td class="special-val">${p.pto||0}</td><td class="special-val">${p.fbp||0}</td><td class="special-val">${p.twocp||0}</td>
  </tr>`).join('');
}
...
function captureSnapshot(){return JSON.parse(JSON.stringify({home:state.home,away:state.away,plays:state.plays,selectedPlayer:state.selectedPlayer,quarterScores:state.quarterScores}));}
function restoreSnapshot(snap){
  Object.assign(state,{home:snap.home,away:snap.away,plays:snap.plays,selectedPlayer:snap.selectedPlayer,quarterScores:snap.quarterScores});
...
async function resetGame() {
  ...
  ['home','away'].forEach(team=>{
    ...
    state[team].pto=0;state[team].fbp=0;state[team].twocp=0;
    state[team].players.forEach(p=>{p.pts=0;p.fgm=0;p.fga=0;p.tpm=0;p.tpa=0;p.ftm=0;p.fta=0;p.or=0;p.dr=0;p.ast=0;p.stl=0;p.blk=0;p.to=0;p.fls=0;p.pto=0;p.fbp=0;p.twocp=0;p.dbStatId=null;});
  });
  state.selectedPlayer=null;state.history=[];state.plays=[];state.quarter='1';state.statsView='home';
  state.quarterScores={'1':null,'2':null,'3':null,'4':null,'OT':null};state.currentGameId=null;viewerMode=false;
  document.querySelectorAll('.quarter-btn').forEach(b=>b.classList.remove('active'));
...
$('btnExportCSV').addEventListener('click', ()=>{
  ...
  const headers=['Team','#','Player','POS','STATUS','PTS','FGM','FGA','3PM','3PA','FTM','FTA','OR','DR','REB','AST','STL','BLK','TO','FLS','PTO','FBP','2CP'];
  const rows=allP.map(p=>[p.team,p.num,p.name,p.pos,p.onCourt?'STARTER':'BENCH',p.pts,p.fgm,p.fga,p.tpm,p.tpa,p.ftm,p.fta,p.or,p.dr,p.or+p.dr,p.ast,p.stl,p.blk,p.to,p.fls,p.pto||0,p.fbp||0,p.twocp||0]);
...
$('btnExportPDF').addEventListener('click', ()=>{
  const allP=[...state.home.players.map(p=>({...p,team:teamName('home')})),...state.away.players.map(p=>({...p,team:teamName('away')}))];
  const rows=allP.map(p=>`<tr><td>${p.team}</td><td>#${p.num} ${p.name}</td><td>${p.pos}</td><td>${p.onCourt?'Starter':'Bench'}</td><td>${p.pts}</td><td>${p.fgm}/${p.fga}</td><td>${p.tpm}/${p.tpa}</td><td>${p.ftm}/${p.fta}</td><td>${p.or+p.dr}</td><td>${p.ast}</td><td>${p.stl}</td><td>${p.blk}</td><td>${p.to}</td><td>${p.fls}</td><td>${p.pto||0}</td><td>${p.fbp||0}</td><td>${p.twocp||0}</td></tr>`).join('');
  const win=window.open('','_blank');
  win.document.write(`<!DOCTYPE html><html><head><title>Box Score</title><style>body{font-family:Arial,sans-serif;padding:24px}table{width:100%;border-collapse:collapse;font-size:.82rem}th,td{border:1px solid #ccc;padding:5px 8px;text-align:center}th{background:#222;color:white}tr:nth-child(even){background:#f5f5f5}</style></head><body><h1>🏀 ${teamName('home')} ${state.home.score} – ${state.away.score} ${teamName('away')}</h1><table><thead><tr><th>Team</th><th>Player</th><th>POS</th><th>Status</th><th>PTS</th><th>FG</th><th>3P</th><th>FT</th><th>REB</th><th>AST</th><th>STL</th><th>BLK</th><th>TO</th><th>FLS</th><th>PTO</th><th>FBP</th><th>2CP</th></tr></thead><tbody>${rows}</tbody></table></body></html>`);
...
function buildScoresheet() {
  ...
  ['1','2','3','4'].forEach((q,i)=>{const n=i+1,qs=state.quarterScores;$(`ssPeriod${n}A`).textContent=qs[q]?qs[q].home:'—';$(`ssPeriod${n}B`).textContent=qs[q]?qs[q].away:'—';});
...
function buildToBoxes(id,count){const el=$(id);if(!el||el.children.length)return;for(let i=0;i<count;i++){const b=document.createElement('div');b.className='ss-to-box';b.addEventListener('click',()=>b.classList.toggle('used'));el.appendChild(b);}}
function buildFoulGrid(id,count){const el=$(id);if(!el||el.children.length)return;for(let i=1;i<=count;i++){const b=document.createElement('div');b.className='ss-foul-box';b.textContent=i;b.addEventListener('click',()=>b.classList.toggle('marked'));el.appendChild(b);}}
function buildRosterTable(tbodyId,players){const tbody=$(tbodyId);tbody.innerHTML='';const max=Math.max(players.length,15);for(let i=0;i<max;i++){const p=players[i];const tr=document.createElement('tr');tr.innerHTML=`<td style="text-align:center;color:var(--muted);font-size:.72rem">${p?`#${p.num}`:''}</td><td>${p?p.name+(p.onCourt?' ⭐':''):''}</td>${[1,2,3,4,5].map(n=>`<td><div class="ss-foul-cell">${p?`<div class="ss-pf-dot${p.fls>=n?' marked':''}"></div>`:''}</div></td>`).join('')}`;if(p)tr.querySelectorAll('.ss-pf-dot').forEach(d=>d.addEventListener('click',()=>d.classList.toggle('marked')));tbody.appendChild(tr);}}
function buildRunningScore(){const tbody=$('ssRunningBody');tbody.innerHTML='';const scoring=[...state.plays].reverse().filter(p=>p.pts>0);const events=[];let hS=0,aS=0;for(const play of scoring){if(play.team==='home'){for(let i=0;i<play.pts;i++){hS++;events.push({h:hS,a:aS});}}else if(play.team==='away'){for(let i=0;i<play.pts;i++){aS++;events.push({h:hS,a:aS});}}}while(events.length<40)events.push({h:null,a:null});for(let row=0;row<10;row++){const tr=document.createElement('tr');let html='';for(let col=0;col<4;col++){const ev=events[col*10+row]||{h:null,a:null};html+=`<td class="home-score">${ev.h??''}</td><td class="away-score">${ev.a??''}</td>`;if(col<3)html+=`<td class="divider"></td>`;}tr.innerHTML=html;tbody.appendChild(tr);}}
